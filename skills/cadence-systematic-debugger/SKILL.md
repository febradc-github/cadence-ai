...
description: Debugging methodology -- reproduce, gather evidence, hypothesize, root-cause, fix, verify -- works on any bug report. Dispatched by /cadence:systematic-debugger or conversate routing only.
...
6. Once the root cause is confirmed, route the fix:
   - **Related to the in_progress item** (the bug lives in that ticket's scope -- same feature or files): the fix is part of that ticket. Dispatch `cadence-coder` with the confirmed root cause, reproduction steps, and intended minimal fix; the fix rides that ticket's diff into `/cadence:review`. Append to the item's `notes`: `debug: <one-line root cause and fix>`.
   - **Unrelated (or nothing in_progress), cadence board present:** do not fix it under someone else's diff. Invoke `cadence-quick` with the bug -- title, confirmed root cause, reproduction, intended fix, tagged `bug`, assignee `claude`. If an unrelated item is `in_progress`, the bug task queues behind it; if nothing is, invoke `cadence-work` with the new id so the fix happens now. Either way it ships through work -> review under its own ticket.
   - **No cadence board:** ad hoc mode -- dispatch `cadence-coder` directly with the root cause and fix approach.
...
8. Dispatch `brain-curator` when the fix changed code (include the touched files -- path, purpose, what changed -- for their `cadence/code/` notes) or the root cause was non-obvious (include a short description); one dispatch covers both.
