# Refine templates

Read when refine reaches its write steps. `<n>` is the ticket number from
`<id>` = `C-<n>`.

## Design doc — cadence/designs/DS-<n>.md

    ---
    type: design
    tags: []
    created: <today, YYYY-MM-DD>
    updated: <today, YYYY-MM-DD>
    related: ["[[<EP-n|US-n>]]"]
    sources: []
    ---

    # <id>: <title> -- Design

    ## Problem
    <problem statement from the dialogue>

    ## Architecture
    <where this sits in the system: affected components, integration
    points, and links to the [[AR-...]] and [[adr-...]] notes it relies
    on or bends -- link only notes that exist (verified in the step 1
    search); name a missing one in plain text instead>

    ## Approach
    <the approach agreed on>

    ## Trade-offs considered
    <alternatives discussed and why not chosen, or "None discussed.">

    ## Acceptance criteria
    - <criterion 1>
    - <criterion 2>

    ## Estimate
    <points> points

    ## Assignee
    <claude|human>

## Backlog entry — appended to cadence/backlog.yml items

    - id: <id>
      title: "<title>"
      status: idea
      points: <points>
      assignee: <claude|human>
      created: <today, YYYY-MM-DD>
      updated: <today, YYYY-MM-DD>

For an epic-sized item, add `type: epic` after `title`. The board holds
tracking fields only — the description lives in the item note, the criteria
in the design doc and later the spec. Never copy prose into YAML.

## Item note — cadence/epics/EP-<n>.md (epic) or cadence/user-stories/US-<n>.md

    ---
    type: <epic|story>
    tags: []
    aliases: ["<id>", "<title>"]
    created: <today, YYYY-MM-DD>
    updated: <today, YYYY-MM-DD>
    related: ["[[DS-<n>]]"]
    ---

    # <id>: <title>

    <one-paragraph description>

    - Design: [[DS-<n>]]

Aliases make the quick switcher and search_notes find the note by board id or
title — but aliases never resolve raw wikilinks, so links always use the
typed name ([[EP-<n>]]/[[US-<n>]]), never [[<id>]].
