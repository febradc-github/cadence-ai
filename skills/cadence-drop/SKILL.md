---
name: cadence-drop
description: Cancels a ticket -- status dropped with a recorded reason; history is never deleted. Dispatched by /cadence:drop or conversate routing only.
...
1. Look up `<id>` (from `$ARGUMENTS`) in `cadence/backlog.yml` and `cadence/sprint.yml`. Not found in either: refuse -- archived items are immutable history and cannot be dropped.
2. If the item is `in_progress`, warn there may be uncommitted implementation changes from `/cadence:work`; ask whether to discard, keep, or commit them separately before dropping. Do not silently strand a half-done diff.
