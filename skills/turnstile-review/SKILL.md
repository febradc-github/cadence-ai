...
3. Read the ticket's spec -- `turnstile/specs/SP-<n>.md` (`<n>` from `<id>` = `C-<n>`), falling back to legacy names (`turnstile/specs/<id>-*-spec.md`, `turnstile/specs/<id>.md`). No spec file means a quick-lane item: criteria live in the item note's "## Acceptance criteria" section. Criteria in neither place: refuse -- the item cannot be reviewed.
