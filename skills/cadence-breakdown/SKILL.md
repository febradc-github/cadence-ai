...
description: Decomposes an epic into stories or an oversized story into tasks; writes child designs and backlog entries after explicit approval. Gate 1a-b. Dispatched by /cadence:breakdown, conversate routing, or refine only.
argument-hint: "[id]"
...
7. Once approved, per the templates reference:
   ...
   - Append each child to `cadence/backlog.yml`, and update the parent there: it is now a container -- if its `status` was `ready`, set it back to `idea` (containers never enter sprints), and set `updated` to today.
...
9. Tell the user the breakdown is recorded and each child needs `/cadence:spec <child-id>` to become `ready`.
