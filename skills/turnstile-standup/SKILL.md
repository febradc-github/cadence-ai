...
1. Read the current sprint -- `turnstile/sprint.yml`, or (legacy boards) the root `turnstile/sprint-*.yml` with `sprint.status: active`. If none exists, tell the user and suggest `/turnstile:sprint-plan`.
...
`turnstile/sprint.yml` (the current sprint), the vault's markdown notes.
...
- **No active sprint:** tell the user, suggest `/turnstile:sprint-plan`.
