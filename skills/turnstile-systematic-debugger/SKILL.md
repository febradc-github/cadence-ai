...
6. Once the root cause is confirmed, route the fix:
   ...
   - **Unrelated (or nothing in_progress), cadence board present:** do not fix it under someone else's diff. Invoke `turnstile-quick` with the bug -- title, confirmed root cause, reproduction, intended fix, tagged `bug`, assignee `claude`. If an unrelated item is `in_progress`, the bug task queues behind it; if nothing is, invoke `turnstile-work` with the new id so the fix happens now. Either way it ships through work -> review under its own ticket.
   - **No cadence board:** ad hoc mode -- dispatch `turnstile-coder` directly with the root cause and fix approach.
...
8. Dispatch `brain-curator` when the fix changed code (include the touched files -- path, purpose, what changed -- for their `turnstile/code/` notes) or the root cause was non-obvious (include a short description); one dispatch covers both.
