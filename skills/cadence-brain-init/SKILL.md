...
description: Bulk-bootstraps cadence/code/ -- one linked note per source file. Dispatched by /cadence:brain-init only.
...
1. Verify the cadence-brain MCP tools respond (any search_notes call). Unavailable: tell the user to reload the session so .mcp.json registers the server, and stop. No cadence/ directory: create nothing; suggest running a cadence command (or /cadence:install-obsidian) first, and stop.
...
7. After all batches return, dispatch brain-curator once more (default model) to stitch: for each top-level source directory, create or extend cadence/architecture/AR-<dir>.md (root-level files: AR-root.md) with a ## Files section linking that directory's file notes in [[slug|path]] form (extend an existing AR note covering the area rather than creating a parallel one); run MOC upkeep for this run's file notes (shared top-level tag code, so moc-code is created or extended once 5+ file notes exist); fix every unresolved link this run introduced, including ones the batch dispatches left behind -- the stitch prompt explicitly authorizes this, overriding the curator's report-don't-fix default; include the slugs of the stale notes deleted in step 6 so the curator runs list_backlinks on each and removes dangling references; finish with list_changed_notes acknowledge: true.
8. Report: notes written, AR notes created or extended, files skipped or failed (a failed batch's files are listed and picked up by a rerun -- they have no notes, so the skip check passes them through), and that /cadence:obsidian-graph shows the result.
