---
name: turnstile-loop-status
description: Reads turnstile/loops/<id>/state.json and renders the current phase, status, and full iteration history in the terminal.
argument-hint: "<id>"
...
> No loop state found for `<id>`. Check the id or run `/turnstile:loop-start` to start a new loop.
...
| Status                  | Message                                              |
|-------------------------|------------------------------------------------------|
| `running`               | Loop is active. Use `/turnstile:loop-status <id>` again to refresh. |
