...
8. For a regular item, tell the user the design is recorded and `/turnstile:spec <id>` is next. For an epic, invoke `turnstile-breakdown` with the epic's id (its own approval gate governs the split).
