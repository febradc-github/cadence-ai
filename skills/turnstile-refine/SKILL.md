...
7. Capture (per the capture config): in `gates` mode (default), dispatch `brain-curator` once after the approval, with bounded input -- the approved design or plan artifact (`DS-<n>`/`PL-<n>`) -- to link it into the vault and record any ADR-worthy choice it contains (a decision between real alternatives with lasting consequences); tell the curator which item notes are affected. In `opportunistic` mode, dispatch only if the dialogue surfaced something worth remembering (a decision record, newly established system shape).
